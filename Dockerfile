...
ENV VIRTUAL_ENV /opt/streamlitgen-env
ENV PATH $VIRTUAL_ENV/bin:$PATH
RUN python -m venv $VIRTUAL_ENV

# Set the working directory.
WORKDIR /workspaces/streamlitgen/
...
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/streamlitgen/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/streamlitgen/ && touch src/streamlitgen/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/streamlitgen/
...
ENTRYPOINT ["/opt/streamlitgen-env/bin/poe"]
CMD ["api"]
